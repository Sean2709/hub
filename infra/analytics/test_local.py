...
a = lf.authorizer_handler(ev("GET", "/stats", headers={"authorization": "Bearer admin-secret-token"}), None)
check(a["isAuthorized"] and a["context"]["role"] == "admin", "authorizer: admin bearer → admin")
a = lf.authorizer_handler(ev("GET", "/stats", headers={"authorization": "Bearer nope"}), None)
check(not a["isAuthorized"], "authorizer: wrong bearer → deny")
...
r = lf.lambda_handler(ev("GET", "/stats", qs={"days": "7"}, headers={"authorization": "Bearer admin-secret-token"}), None)
...
r = lf.lambda_handler(ev("GET", "/stats", qs={"days": "9999"}, headers={"authorization": "Bearer admin-secret-token"}), None)
...
check(lf.lambda_handler(ev("GET", "/health", headers={"authorization": "Bearer admin-secret-token"}), None)["statusCode"] == 200, "health: admin ok")
check(lf.lambda_handler(ev("GET", "/nope", headers={"authorization": "Bearer admin-secret-token"}), None)["statusCode"] == 404, "unknown path → 404")
